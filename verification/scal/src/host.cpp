...
constexpr void initialize_data(Number *x, Number &alpha,
                               std::uint64_t size) {
...
int main(int argc, char *argv[]) {
    ...
    xrt::kernel mm2s = xrt::kernel(device, uuid, "scal_mm2s");
    xrt::kernel s2mm = xrt::kernel(device, uuid, "scal_s2mm");
    ...
    initialize_data(x, alpha, args.size);
    ...
    cblas_scopy(args.size, x, 1, result, 1);
    cblas_sscal(args.size, alpha, result, 1);
